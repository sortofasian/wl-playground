...
pub mod wp_presentation {
    ...
    pub enum Request {
        ...
        Feedback {
            ...
            callback: Main<super::wp_presentation_feedback::WpPresentationFeedback>,
        },
    }
            ...
            match opcode {
                ...
                1 => {
                    ...
                    Ok(Request::Feedback {
                        ...
                        .into(),
                        callback: {
                            let me = Resource::<WpPresentation>::from_c_ptr(obj as *mut _);
                            me . make_child_for :: < super :: wp_presentation_feedback :: WpPresentationFeedback > (_args [1] . n) . unwrap ()
                        },
                    })
                }
                _ => return Err(()),
    ...
    impl super::MessageGroup for Event {
        ...
        fn into_raw(self, sender_id: u32) -> Message {
            match self {
                Event::ClockId { clk_id } => Message {
                    sender_id: sender_id,
                    opcode: 0,
                    args: smallvec![Argument::Uint(clk_id),],
                },
            }
        }
        unsafe fn from_raw_c(
        ...
        {
            match self {
                Event::ClockId { clk_id } => {
                    let mut _args_array: [wl_argument; 1] = unsafe { ::std::mem::zeroed() };
                    _args_array[0].u = clk_id;
                    f(0, &mut _args_array)
                }
            }
        }
    ...
    pub struct WpPresentation(Resource<WpPresentation>);
    ...
    impl WpPresentation {
        ...
        pub fn clock_id(&self, clk_id: u32) -> () {
            let msg = Event::ClockId { clk_id: clk_id };
            self.0.send(msg);
        }
    }
...
pub mod wp_presentation_feedback {
    ...
    impl super::MessageGroup for Event {
        ...
        fn into_raw(self, sender_id: u32) -> Message {
            match self {
                Event::SyncOutput { output } => Message {
                    sender_id: sender_id,
                    opcode: 0,
                ...
                Event::Presented {
                    ...
                    args: smallvec![
                        ...
                        Argument::Uint(refresh),
                        Argument::Uint(seq_hi),
                        Argument::Uint(seq_lo),
            ...
            F: FnOnce(u32, &mut [wl_argument]) -> T,
        {
            match self {
                ...
                Event::Presented {
                    ...
                    _args_array[3].u = refresh;
                    _args_array[4].u = seq_hi;
                    _args_array[5].u = seq_lo;
                    _args_array[6].u = flags.to_raw();
                    f(1, &mut _args_array)
                }
                Event::Discarded => {
    ...
    #[derive(Clone, Eq, PartialEq)]
    pub struct WpPresentationFeedback(Resource<WpPresentationFeedback>);
    impl AsRef<Resource<WpPresentationFeedback>> for WpPresentationFeedback {
        #[inline]
        fn as_ref(&self) -> &Resource<Self> {
            &self.0
        }
    }
    impl From<Resource<WpPresentationFeedback>> for WpPresentationFeedback {
        #[inline]
        fn from(value: Resource<Self>) -> Self {
    ...
    impl WpPresentationFeedback {
        #[doc = "presentation synchronized to this output\n\nAs presentation can be synchronized to only one output at a\ntime, this event tells which output it was. This event is only\nsent prior to the presented event.\n\nAs clients may bind to the same global wl_output multiple\ntimes, this event is sent for each bound instance that matches\nthe synchronized output. If a client has not bound to the\nright wl_output global at all, this event is not sent."]
        pub fn sync_output(&self, output: &super::wl_output::WlOutput) -> () {
            let msg = Event::SyncOutput {
                output: output.clone(),
            };
            self.0.send(msg);
        }
